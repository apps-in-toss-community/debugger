...
function warnIfEnvDiffersFromFile(
  ...
  if (envSecret !== stored) {
    logFn(
      `[@ait-co/debugger] AIT_DEBUG_TOTP_SECRET (from environment) differs from the project-local relay secret; ` +
...
async function readAndInject(
  ...
  try {
    ...
    throw new Error(`[@ait-co/debugger] relay 시크릿 파일 읽기 실패: ${msg}`);
  }

  if (!isValidRelayAuthSecret(stored)) {
    // Stored value is corrupt — re-mint over the same path.
    logFn('[@ait-co/debugger] relay 시크릿 파일의 값이 유효하지 않습니다. 재생성합니다.\n');
...
async function mintAndPersist(
  ...
  if (!isValidRelayAuthSecret(secret)) {
    throw new Error(
      '[@ait-co/debugger] 내부 오류: mint된 시크릿이 유효성 검사를 통과하지 못했습니다.',
    );
  }
  ...
  try {
    ...
    if ((err as NodeJS.ErrnoException).code === 'EEXIST') {
      ...
      try {
        ...
        throw new Error(`[@ait-co/debugger] relay 시크릿 파일 읽기 실패(경합): ${msg}`);
      }
      if (!isValidRelayAuthSecret(stored)) {
        throw new Error('[@ait-co/debugger] relay 시크릿 파일이 경합 후에도 유효하지 않습니다.');
      }
      env.AIT_DEBUG_TOTP_SECRET = stored;
      return;
    }
  ...
  logFn(
    `[@ait-co/debugger] relay 인증 시크릿을 생성해 프로젝트의 ${RELAY_SECRET_FILE_NAME} 파일에 저장했습니다 (권한 0600).\n` +
