...
/**
 * Reads the mocked SDK namespace with the shape this suite planted above.
 *
 * The cast is load-bearing: the devDependency pins the 2.x line, whose web
 * surface has no `setScreenAwakeMode` at all (it arrives in 3.0). That is
 * exactly why `attach.ts` reaches the API through a runtime probe rather than a
 * static import — a static import would not compile here, and would break every
 * 2.x consumer at runtime. The mock supplies the symbol for these tests.
 */
async function mockedFramework(): Promise<{ setScreenAwakeMode: ReturnType<typeof vi.fn> }> {
  return (await import('@apps-in-toss/web-framework')) as unknown as {
    setScreenAwakeMode: ReturnType<typeof vi.fn>;
  };
}
...
describe('deriveTargetScriptUrl', () => {
  ...
  beforeEach(async () => {
    vi.resetModules();
    ({ deriveTargetScriptUrl } = await import('../attach.js'));
  });
...
describe('maybeAttach', () => {
  ...
  beforeEach(async () => {
    ...
    ({ maybeAttach } = await import('../attach.js'));
  });
...
describe('keepAwake behavior', () => {
  ...
  beforeEach(async () => {
    ...
    const framework = await mockedFramework();
...
describe('installRelayWsObserver', () => {
  ...
  beforeEach(async () => {
    ...
    ({ installRelayWsObserver } = await import('../attach.js'));
  });
...
describe('reportWebViewType (#580)', () => {
  ...
  beforeEach(async () => {
    vi.resetModules();
    ({ reportWebViewType } = await import('../attach.js'));
  });
...
describe('detachDebugSurface (#748)', () => {
  ...
  beforeEach(async () => {
    ...
    ({ detachDebugSurface } = await import('../attach.js'));
  });
  ...
  it('restores screen sleep — setScreenAwakeMode({ enabled: false })', async () => {
    detachDebugSurface();
    // The SDK is reached through a runtime probe now, not a static import, so a
    // cold teardown (no prior attach to warm the probe) dispatches the release
    // on a later microtask. The warm path — the one an unloading page takes —
    // is synchronous; see sdk-probe.ts.
    await vi.waitFor(() => expect(setScreenAwakeMode).toHaveBeenCalledWith({ enabled: false }));
  });
  ...
  it('is idempotent — a second call does not disable awake again', async () => {
    detachDebugSurface();
    detachDebugSurface();
    await vi.waitFor(() => expect(setScreenAwakeMode).toHaveBeenCalledTimes(1));
    // And it stays at one after the probe has had every chance to settle.
    await Promise.resolve();
...
describe('detachDebugSurface — #749 teardown wiring', () => {
  ...
  beforeEach(async () => {
    ...
    setScreenAwakeMode.mockResolvedValue({ enabled: false });
    ({ detachDebugSurface } = await import('../attach.js'));
    ({ installBridgeObserver } = await import('../bridge-observer.js'));
  });

  afterEach(() => {
    // `delete` rather than `= undefined` — see bridge-observer.test.ts.
    delete bw().__appsInTossNativeBridge;
    delete bw().__ait_bridge;
    delete bw().__ait_indicator;
  });
...
describe('graceful detach on relay WS close (#748)', () => {
  ...
  it('tears down after the grace window on a non-4401 relay close', async () => {
    addBadge();
    installRelayWsObserver(RELAY_URL);
    const ws = new window.WebSocket('wss://relay.example.com/target/abc');
    ws.dispatchEvent(closeEventWithCode(1006));
  ...
  it('tears down immediately on a 4401 relay close (terminal — no grace)', async () => {
    addBadge();
    installRelayWsObserver(RELAY_URL);
    const ws = new window.WebSocket('wss://relay.example.com/at/123456/target/abc');
    ws.dispatchEvent(closeEventWithCode(RELAY_AUTH_REJECT_CLOSE_CODE));

    // "Immediately" is about the grace window, not the awake release: the badge
    // is gone in this tick, and the probe-dispatched release lands right after.
    expect(document.getElementById('__ait_debug_indicator')).toBeNull();
    await vi.waitFor(() => expect(setScreenAwakeMode).toHaveBeenCalledWith({ enabled: false }));
  });
  ...
  it('runs teardown on pagehide (beforeunload-safe path)', async () => {
    addBadge();
    installRelayWsObserver(RELAY_URL);
