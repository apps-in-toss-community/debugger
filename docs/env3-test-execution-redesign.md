...
이 문서는 분리 전 `devtools` repo에서 작성됐다(2026-06-29). devtools#818 3-패키지 분리로 attach orchestrator·test-runner 구현 표면이 이 repo(`packages/debugger`)로 옮겨오면서 문서도 함께 이관됐다. 추적 이슈는 이 repo #10.
...
2. **고전 npm script 경로** — 테스트 구동이 MCP 전용이면 안 된다. `pnpm test:env3` 같은 클래식 스크립트로도 실행 가능해야 한다. `debugger-test` bin이 `package.json`에 이미 있으나(`packages/debugger/src/test-runner/cli.ts`) `main()`이 stub다.
...
|---|---|---|---|
| `discoverTestFiles(patterns, cwd)` | `packages/debugger/src/test-runner/discover.ts:29` | `(string[], string) → Promise<string[]>` (절대경로) | 없음 (순수 Node IO) |
| `bundleTestFile(absPath, opts)` | `packages/debugger/src/test-runner/bundle.ts:276` | esbuild iife 번들 + SDK→`window.__sdk` redirect | 없음 (esbuild lazy import) |
| `injectAndRunBundle(conn, code, timeoutMs)` | `packages/debugger/src/test-runner/rpc.ts:110` | `Runtime.evaluate`로 주입 + RunReport 수확 | `CdpConnection.send`만 |
| `runTestFilesOverRelay(conn, files, opts)` | `packages/debugger/src/test-runner/relay-worker.ts:82` | 파일 순차 bundle→inject→collect, `RelayRunReport` 반환 | `CdpConnection`만 |
| `runWithConnection(conn, files, opts)` | `packages/debugger/src/test-runner/cli.ts:71` | `runTestFilesOverRelay` 얇은 래퍼 + printSummary | `CdpConnection`만 |
...
`__AIT_CELL__`은 **`@ait-co/debugger`가 아니라 sdk-example가 소유**한다. `sdk-example/src/test/aitCapture.ts`:
...
devtools 쪽엔 `__AIT_CELL__` 참조가 0건이다(grep 확인) — devtools는 cell **모양**을 모르고 알 필요도 없다.

### 1.5 소유권 — `test:env3` 스크립트는 어느 패키지?

확인 결과:

- `packages/debugger/package.json` `bin`: `"debugger-test": "./dist/test-runner/bin.js"` (있음).
...
**결론**: 도구(`debugger-test` bin)는 `@ait-co/debugger`(이 repo)가 소유하고, **스크립트(`test:env3`)는 sdk-example가 소유**한다 — sdk-example가 `"test:env3": "debugger-test 'src/**/*.ait.test.ts' --cell-platform ios"` 식으로 자기 글롭/cell을 박아 호출한다. `@ait-co/debugger`는 cell 모양을 모르는 일반 bin, sdk-example는 자기 cell 축을 아는 호출자. (이건 `bundle:ait`가 `@apps-in-toss/cli`의 `ait` bin을 sdk-example 스크립트가 부르는 구조와 동형이다.)

---

## 2. 답: attach orchestrator 추출 경계 (질문 1)

### 2.1 추출할 모듈 — `packages/debugger/src/mcp/attach-orchestrator.ts` (신규)
...
위치는 `packages/debugger/src/mcp/`다(`packages/debugger/src/test-runner/`가 아니라). 이유: QR/터널/TOTP는 debug-MCP 인프라이고, `qr-http-server`·`tunnel`·`totp`·`deeplink`가 전부 `packages/debugger/src/mcp/`에 있다. test-runner는 이 orchestrator를 **소비**한다(역의존 아님). install-graph 불변식(§3.3)도 이 위치가 안전하다 — orchestrator는 이미 MCP 데몬 그래프에 있는 모듈만 끌어온다.

### 2.2 `AttachDeps` — 6개 클로저 변수의 명시적 승격

```ts
// packages/debugger/src/mcp/attach-orchestrator.ts
...
### 4.1 `@ait-co/debugger` 쪽 — 일반 메커니즘 (cell 모양 모름)

`@ait-co/debugger`는 `__AIT_CELL__`의 sdk-example-특정 모양을 몰라야 한다(§1.4). 일반 주입 helper를 추가:

```ts
// packages/debugger/src/test-runner/cell.ts (신규) — react-free, CdpConnection만 의존
...
| PR | 범위 | 검증 | 위험 |
|---|---|---|---|
| **PR1 — orchestrator 추출** (행동 무변경 리팩터) | §2. `packages/debugger/src/mcp/attach-orchestrator.ts` 신규: `mint/buildTotpMeta/prepareAttach/renderAndMaybeWait/waitWithRemint/waitForAttachWithEvents`를 `AttachDeps` 시그니처로 모듈 레벨화. `createDebugServer`는 자기 클로저 변수로 `attachDeps` 조립해 호출하는 래퍼로. 공유 타입 export. | 기존 debug-server start_attach 테스트(4-path/re-mint/timeout) green. `pnpm typecheck` 두 라인. `check:mcp-react-free`. | 낮음 — 순수 이동. 클로저→deps 누락 시 컴파일 에러로 잡힘. |
| **PR2 — run_tests auto-attach** | §3 + §4.1·4.2. run_tests 핸들러에 attach 분기, `packages/debugger/src/test-runner/cell.ts`(`injectGlobals`), descriptor 인자(scheme_url/projectRoot/cell). | "이미 연결됨" 경로 회귀 테스트(4a 무변경). page-0 + scheme_url → prepare→wait 호출 검증(fake conn). cell 주입 evaluate 검증. | 중 — 기존 수동 플로우 호환(§6 위험 1). page-0 분기만 발동하게 가드. |
| **PR3 — CLI 배선 + test:env3** | §5.2·5.3. `cli.ts main()` 구현(bootRelayFamily + orchestrator + run core + cell). sdk-example `test:env3` 스크립트(별 PR, sdk-example repo). | `debugger-test --help`/glob discovery 기존 테스트 유지. CLI attach는 실기기라 수동 QA(폰 1대) — 단위는 connection 조립까지. install-graph 불변식 §3.3. | 중 — install-graph(§5.4). 실기기 의존이라 CI 자동화 불가, 수동 검증. |
...
"test:env3": "debugger-test 'src/**/*.ait.test.ts' --cell-platform ios --scheme-url \"$AIT_SCHEME_URL\""
```

- glob·cell·scheme은 **sdk-example가** 자기 맥락으로 박는다(`@ait-co/debugger`는 모름).
- `AIT_SCHEME_URL`은 `ait deploy --scheme-only` 산출 intoss-private URL(per-run). cell platform은 검증 대상 폰에 맞춰 ios/android.
- `@ait-co/debugger`는 `debugger-test` bin을 sdk-example의 devDep `@ait-co/debugger`로 제공(현재 없음 → PR3와 함께 sdk-example가 명시 devDep 추가). `bundle:ait`가 `@apps-in-toss/cli`의 `ait` bin을 부르는 것과 동형 구조.

### 5.4 install-graph 불변식 충돌 점검 (§질문 3 단서)

이 repo(`debugger`) `CLAUDE.md`의 **react-free·MCP-only install 불변식**과 충돌하는가?

- `debugger-test` bin(`dist/test-runner/bin.js`)이 끌어오는 것: `discover`(순수 Node), `bundle`(esbuild **lazy import**), `rpc`/`relay-worker`(CdpConnection만), 그리고 PR3에서 추가되는 `bootRelayFamily`/`startChiiRelay`/`ChiiCdpConnection`/`attach-orchestrator`. 이들은 전부 **이미 MCP 데몬 그래프에 있는 모듈**(`chii`·`ws`·`cloudflared`·`qrcode`는 `dependencies`, 동적 import). **react/react-dom은 안 끌어온다** — orchestrator의 QR 대시보드는 `qrHttpServer` 주입 시에만이고 CLI는 미주입(text QR = `qrcode-terminal`). esbuild는 lazy(bundle.ts:60-66 주석)라 import 시점엔 그래프 밖.
- 따라서 `debugger-test`는 react-free 유지. 단 PR3는 `packages/debugger/scripts/check-test-runner-dist.sh`(이미 package.json에 배선)에 **test-runner CLI 번들의 react 부재 + esbuild lazy 유지** 체크를 추가/확장해 회귀를 CI 강제한다.
...
- run core 재사용 경계: `packages/debugger/src/test-runner/relay-worker.ts`(`runTestFilesOverRelay`), `packages/debugger/src/test-runner/cli.ts`(`runWithConnection`).
- 추출 대상 클로저: `packages/debugger/src/mcp/debug-server.ts:707/737/971/1011`, 순수 primitive `:579`(`waitForAttachWithEvents`).
- connection boot: `packages/debugger/src/mcp/debug-server.ts:2065`(`bootRelayFamily`), `:1887`(`createRelayConnection`), `packages/debugger/src/mcp/chii-relay.ts:314`(`startChiiRelay`).
- cell 소유: `sdk-example/src/test/aitCapture.ts`(`__AIT_CELL__` sdkLine/platform), env `AIT_CELL_PLATFORM`.
- install-graph 불변식: 이 repo(`debugger`) `CLAUDE.md`의 Invariants 섹션 + `packages/debugger/scripts/check-mcp-react-free.sh`·`packages/debugger/scripts/check-test-runner-dist.sh`.
- 설계 승인 후 구현은 §5 PR 분할로. cross-repo(sdk-example)는 PR 분리(umbrella §6.2).
